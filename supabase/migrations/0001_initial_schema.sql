...
--
-- NOTE on ordering: SQL functions are parsed/validated at creation time, so any
-- helper that references a table is declared AFTER that table exists.
-- ============================================================================
...
-- ============================================================================
-- RLS helper functions (SECURITY DEFINER to bypass RLS and avoid policy
-- recursion). Declared here, after the tables they read.
-- ============================================================================
create or replace function public.is_member(p_business uuid)
returns boolean language sql stable security definer set search_path = public as $$
  select exists (
    select 1 from public.business_members m
    where m.business_id = p_business and m.user_id = public.current_user_id()
  );
$$;
...
returns boolean language sql stable security definer set search_path = public as $$
  select exists (
    ...
    where me.user_id = public.current_user_id() and them.user_id = p_target
  );
$$;
...
create policy profiles_select on public.profiles
  for select using (id = public.current_user_id() or public.shares_business(id));
...
-- businesses: members read all; public may read ACTIVE businesses (storefront
-- info, not sensitive). Owner creates; admins update/delete.
...
-- queues: members read; public reads queues of ACTIVE businesses (join page).
-- Admins create/delete; any member may toggle/operate a queue.
...
-- go through the SECURITY DEFINER RPCs below (heart of the secure boundary).
...
create or replace function public.join_queue(
  p_queue_id uuid, p_name text, p_party integer, p_phone text default null
)
returns table (
...
begin
  ...
  return query
  select v_entry.join_token, v_entry.status, public.people_ahead(v_entry),
...
returns table (
  ...
  position_in_line integer, queue_name text, business_name text, customer_name text, created_at timestamptz
)
...
begin
  ...
  select v_entry.status, public.people_ahead(v_entry),
         public.people_ahead(v_entry) * v_queue.avg_service_minutes, v_entry.position,
         v_queue.name, v_biz.name, v_entry.customer_name, v_entry.created_at;
end;
$$;
...
begin
  update public.queue_entries
   ...
   where join_token = p_join_token and status in ('waiting', 'called', 'serving')
  returning * into v_entry;
  if found then
    insert into public.activity (business_id, actor_id, type, payload)
  ...
  select status into v_status from public.queue_entries where join_token = p_join_token;
  return coalesce(v_status, 'cancelled'::public.entry_status);
end;
$$;
